      ...
      >
        ...
        >
          ...
          <div className="relative flex-shrink-0 flex items-center justify-center w-full min-h-[40vh]">
            ...
            {/* 画像とボタンのグループコンテナ */}
            <div className="relative max-w-full max-h-[65vh] inline-block">
              ...
              <img
                ...
                className={`max-w-full max-h-[65vh] object-contain rounded-lg shadow-2xl select-none transition-opacity duration-300 ${
                  (isImageLoading || !isSynced) ? 'absolute top-0 left-0 w-full h-full opacity-0 pointer-events-none' : 'opacity-100'
                }`}
              />
              ...
              {activeIsOwner && (
                <div 
                  onClick={(e) => e.stopPropagation()}
                  className="absolute top-4 right-4 flex items-center gap-2 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-200 z-20"
                >
                  {/* 編集ボタン */}
                  <button
