...
// 1. 承認待ちの申請一覧取得
router.get('/role-requests', requirePermission('management', 'read'), async (_req: Request, res: Response) => {
  try {
    const { data: nullRoleMappings, error } = await supabase
      .from('user_role_mappings')
    ...
    pending.forEach(p => (p.metadata?.department_ids || []).forEach((id: string) => allDeptIds.add(id)));
    ...
    const enriched = await Promise.all(pending.map(async (p) => {
      ...
      const departmentIds: string[] = p.metadata?.department_ids || [];
      return {
        id: p.id,
        user_id: p.user_id,
...
router.post('/role-requests/:id/approve', requirePermission('management', 'write'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const departmentIds: string[] = mapping.metadata.department_ids || [];
    if (requestedRole === 'smiring_member' && departmentIds.length > 0) {
      const inserts = departmentIds.map((deptId: string) => ({
        user_id: mapping.user_id,
        department_id: deptId
      }));
      const { error: deptError } = await supabase.from('member_department_mappings').insert(inserts);
...
router.delete('/role-requests/:id', requirePermission('management', 'write'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
// 2-1. 新規権限タイプ作成
router.post('/permission-types', requirePermission('management', 'write'), async (req: Request, res: Response) => {
  try {
...
// 2-2. 権限タイプ更新
router.patch('/permission-types/:id', requirePermission('management', 'write'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { type, description } = req.body;
    if (!type || !type.trim()) {
...
// 2-3. 権限タイプ削除
router.delete('/permission-types/:id', requirePermission('management', 'write'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
