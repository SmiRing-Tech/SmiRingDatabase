...
exports.queueIndexWork = queueIndexWork;
exports.deleteSearchIndex = deleteSearchIndex;
exports.deleteSearchIndexByMetadata = deleteSearchIndexByMetadata;
exports.queueGalleryImageIndexWork = queueGalleryImageIndexWork;
const supabase_1 = require("./supabase");
const ai_1 = require("./ai");
/**
 * 検索インデックスへの登録・更新をバックグラウンドで実行する
 */
async function queueIndexWork(params) {
    ...
    try {
        ...
        const [localVector, geminiVector] = await Promise.all([
            (0, ai_1.getLocalEmbedding)(content, false), // 文書用
            (0, ai_1.getGeminiEmbedding)(content, false), // 文書用
        ]);
        // 2. インデックス保存 (Upsert)
        let deleteQuery = supabase_1.supabase
 ...
 */
async function deleteSearchIndex(source_type, source_id) {
 ...
 */
async function deleteSearchIndexByMetadata(source_type, metadataFilters) {
 ...
 */
async function queueGalleryImageIndexWork(galleryId, descriptions, visibility, metadata) {
    try {
        ...
        for (let i = 0; i < descriptions.length; i++) {
            ...
            if (!line.trim())
                continue;
            const [localVector, geminiVector] = await Promise.all([
                (0, ai_1.getLocalEmbedding)(line, false),
                (0, ai_1.getGeminiEmbedding)(line, false),
            ]);
