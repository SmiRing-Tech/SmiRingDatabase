...
const roomService = LIVEKIT_URL && LIVEKIT_API_KEY && LIVEKIT_API_SECRET
    ? new livekit_server_sdk_1.RoomServiceClient(LIVEKIT_URL, LIVEKIT_API_KEY, LIVEKIT_API_SECRET)
    : null;
const webhookReceiver = LIVEKIT_API_KEY && LIVEKIT_API_SECRET
    ? new livekit_server_sdk_1.WebhookReceiver(LIVEKIT_API_KEY, LIVEKIT_API_SECRET)
    : null;
...
function getCanonicalThreadId(identities) {
 ...
 */
async function mintLiveKitToken(userId, room, usernameOverride) {
...
async function getDisplayProfile(userId, fallback) {
...
async function isSmiRingMemberHost(userId) {
    const { data: mapping } = await supabase_1.supabase
    ...
    if (!roleData?.id)
        return false;
    const { data: fallbackMapping } = await supabase_1.supabase
        .from('user_role_mappings')
        .select('user_id')
        .eq('user_id', userId)
...
async function requireMiniRoomHost(req, res, next) {
    try {
...
async function getActiveMiniRooms(mainRoomId) {
...
function serializeMiniRooms(rows) {
...
async function findParticipantCurrentRoom(candidateRoomIds, identity) {
    if (!roomService)
        return null;
    const results = await Promise.all(candidateRoomIds.map(async (roomId) => {
        try {
...
async function broadcastMiniRoomSync(mainRoomId, rooms, allowSelfAssign) {
    if (!roomService)
        return;
    ...
    await Promise.all(targets.map((roomId) => roomService
        .sendData(roomId, payload, livekit_server_sdk_1.DataPacket_Kind.RELIABLE, { topic: 'miniroom_sync' })
 ...
 */
async function isMainRoomSessionEmpty(mainRoomId) {
    if (!(await isRoomEmpty(mainRoomId)))
        return false;
    ...
    if (miniRooms === null)
        return false; // Inconclusive — don't risk deleting active mini rooms.
    if (miniRooms.length === 0)
        return true;
    try {
...
async function cleanupStaleRoomData(mainRoomId) {
    const { error: chatError } = await supabase_1.supabase.from('connect_chat_messages').delete().eq('room_id', mainRoomId);
    if (chatError) {
    ...
    const { error: miniError } = await supabase_1.supabase
        .from('connect_miniroom_rooms')
        .delete()
        .eq('main_room_id', mainRoomId);
    if (miniError) {
...
router.get('/api/connect/rooms/:roomId/messages', authenticate_1.authenticate, async (req, res) => {
    try {
...
router.post('/api/connect/rooms/:roomId/messages', authenticate_1.authenticate, async (req, res) => {
    try {
        const { roomId } = req.params;
        if (!isValidRoomName(roomId)) {
            return res.status(400).json({ error: 'ルーム名が不正です' });
...
router.get('/api/connect/rooms/:roomId/miniroom', authenticate_1.authenticate, async (req, res) => {
    try {
        const { roomId } = req.params;
        if (!isValidRoomName(roomId)) {
            return res.status(400).json({ error: 'ルーム名が不正です' });
...
router.post('/api/connect/rooms/:roomId/miniroom', authenticate_1.authenticate, requireMiniRoomHost, async (req, res) => {
    try {
        const { roomId } = req.params;
        if (!isValidRoomName(roomId)) {
            return res.status(400).json({ error: 'ルーム名が不正です' });
        ...
        const allowSelfAssign = typeof requestedAllowSelfAssign === 'boolean'
        ...
        try {
            for (const name of names) {
                ...
                const { error } = await supabase_1.supabase.from('connect_miniroom_rooms').insert([
                    {
                        id,
                        main_room_id: roomId,
                        name,
            ...
            await Promise.all(created.map((r) => Promise.all([
                roomService.deleteRoom(r.id).catch(() => { }),
                supabase_1.supabase.from('connect_miniroom_rooms').delete().eq('id', r.id),
            ])));
...
router.get('/api/connect/rooms/:roomId/miniroom/participants', authenticate_1.authenticate, requireMiniRoomHost, async (req, res) => {
    try {
        const { roomId } = req.params;
        if (!isValidRoomName(roomId)) {
            return res.status(400).json({ error: 'ルーム名が不正です' });
...
router.post('/api/connect/rooms/:roomId/miniroom/move', authenticate_1.authenticate, async (req, res) => {
    try {
        const { roomId } = req.params;
        if (!isValidRoomName(roomId)) {
            return res.status(400).json({ error: 'ルーム名が不正です' });
        ...
        const fromRoom = await findParticipantCurrentRoom([roomId, ...miniRooms.map((r) => r.id)], targetIdentity);
        if (!fromRoom) {
        ...
        const notifyPayload = Buffer.from(JSON.stringify({
        ...
        try {
            await roomService.sendData(fromRoom, notifyPayload, livekit_server_sdk_1.DataPacket_Kind.RELIABLE, {
...
router.post('/api/connect/rooms/:roomId/miniroom/close', authenticate_1.authenticate, requireMiniRoomHost, async (req, res) => {
    try {
        const { roomId } = req.params;
        if (!isValidRoomName(roomId)) {
            return res.status(400).json({ error: 'ルーム名が不正です' });
        ...
        const delayMs = 3000;
        await Promise.all(targets.map(async (miniRoom) => {
            let participants = [];
            try {
            ...
            await Promise.all(participants.map(async (p) => {
                try {
                    ...
                    await roomService.sendData(miniRoom.id, notifyPayload, livekit_server_sdk_1.DataPacket_Kind.RELIABLE, {
        ...
        const { error } = await supabase_1.supabase.from('connect_miniroom_rooms').delete().in('id', idsToRemove);
        if (error) {
...
router.post('/api/connect/webhook', async (req, res) => {
    if (!webhookReceiver) {
    ...
    try {
        ...
        const maybeDone = (event.event === 'room_finished' && event.room?.name) ||
