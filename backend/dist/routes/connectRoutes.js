...
router.post('/api/connect/rooms/:roomId/miniroom/move', authenticate_1.authenticate, async (req, res) => {
    try {
        ...
        const delayMs = 4000;
        const notifyPayload = Buffer.from(JSON.stringify({ type: 'miniroom_notify', destinationRoomId, destinationName, delayMs }), 'utf8');
        try {
...
router.post('/api/connect/rooms/:roomId/miniroom/close', authenticate_1.authenticate, requireMiniRoomHost, async (req, res) => {
    try {
        ...
        const delayMs = 3000;
        const notifyPayload = Buffer.from(JSON.stringify({ type: 'miniroom_notify', destinationRoomId: roomId, destinationName: 'メインルーム', delayMs }), 'utf8');
...
router.post('/api/connect/webhook', async (req, res) => {
    ...
    try {
        ...
        const shouldDelete = (event.event === 'room_finished' && event.room?.name) ||
