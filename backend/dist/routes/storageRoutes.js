...
async function ensureJpegBuffer(buffer, mimetype, originalname) {
...
router.post('/api/gallery/upload', authenticate_1.authenticate, upload.single('file'), async (req, res) => {
    try {
        // ファイルの存在チェック
        if (!req.file) {
        ...
        const largeBuffer = await (0, sharp_1.default)(processedBuffer)
            .resize(1920, 1920, { fit: 'inside', withoutEnlargement: true })
            .jpeg({ quality: 80, progressive: true })
            .toBuffer();
        const thumbBuffer = await (0, sharp_1.default)(processedBuffer)
        ...
        const { data: gallery, error: insertError } = await supabase_1.supabase
            .from('gallery')
            .insert({
            user_id: req.user.id,
...
// 🖼️ ギャラリー一覧取得 API
// ==========================================
router.get('/api/gallery', authenticate_1.authenticate, async (req, res) => {
    try {
        ...
        const imageTypes = req.query.imageTypes ? req.query.imageTypes.split(',') : undefined;
        ...
        const galleriesWithUrls = await Promise.all((galleries || []).map(async (g) => {
            ...
            if (profile?.avatar_id) {
                const { data: avatar } = await supabase_1.supabase.from('gallery').select('storage_path').eq('id', profile.avatar_id).single();
                if (avatar) {
                    const command = new client_s3_1.GetObjectCommand({ Bucket: BUCKET_NAME, Key: avatar.storage_path });
                    avatarUrl = await (0, s3_request_presigner_1.getSignedUrl)(r2, command, { expiresIn: 3600 });
                }
            }
            let viewUrl = '';
            let thumbnailUrl = '';
            try {
                ...
                viewUrl = await (0, s3_request_presigner_1.getSignedUrl)(r2, command, { expiresIn: 3600 });
                // サムネイル画像
                // もし thumbnail_path がない場合は、フォールバックとしてオリジナルを表示する
                const thumbKey = g.thumbnail_path || g.storage_path;
                const thumbCommand = new client_s3_1.GetObjectCommand({
                    Bucket: BUCKET_NAME,
                    Key: thumbKey,
                });
                thumbnailUrl = await (0, s3_request_presigner_1.getSignedUrl)(r2, thumbCommand, { expiresIn: 3600 });
            }
            catch (err) {
...
// ==========================================
router.get('/api/gallery/user/:userId', authenticate_1.authenticate, async (req, res) => {
    try {
        const targetUserId = req.params.userId;
        const isOwner = req.user.id === targetUserId;
        let query = supabase_1.supabase.from('gallery').select('*').eq('user_id', targetUserId);
        if (!isOwner) {
            // 他人の場合は公開設定のもののみ表示
        ...
        const sortedGalleries = (galleries || []).sort((a, b) => {
            if (a.image_type === 'avatar' && b.image_type !== 'avatar')
                return -1;
            if (a.image_type !== 'avatar' && b.image_type === 'avatar')
                return 1;
            return 0; // あとは created_at の降順
        });
        const galleriesWithUrls = await Promise.all(sortedGalleries.map(async (item) => {
            const command = new client_s3_1.GetObjectCommand({
                Bucket: BUCKET_NAME,
                Key: item.storage_path,
            });
            const viewUrl = await (0, s3_request_presigner_1.getSignedUrl)(r2, command, { expiresIn: 3600 });
            // サムネイル
            ...
            const thumbnailUrl = await (0, s3_request_presigner_1.getSignedUrl)(r2, thumbCommand, { expiresIn: 3600 });
...
// 🔍 ギャラリー個別取得 API
// ==========================================
router.get('/api/gallery/:id', authenticate_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const { data: gallery, error: fetchError } = await supabase_1.supabase
            .from('gallery')
            .select('*')
            .eq('id', id)
            .single();
...
// ✏️ ギャラリー情報更新 API
// ==========================================
router.patch('/api/gallery/:id', authenticate_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const { image_type, visibility, description } = req.body;
        ...
        // galleryテーブルのレコードを更新
        const { data: updatedGallery, error: updateError } = await supabase_1.supabase
            .from('gallery')
            .update({
...
// 🗑️ ギャラリー削除 API
// ==========================================
router.delete('/api/gallery/:id', authenticate_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (deleteError)
            throw deleteError;
        // 🔍 検索インデックスからも削除
        await (0, vectorIndexer_1.deleteSearchIndex)('gallery_image', id);
...
// ==========================================
router.post('/api/forms/attachments/upload', authenticate_1.authenticate, attachmentUpload.single('file'), async (req, res) => {
    try {
        if (!req.file)
            return res.status(400).json({ error: 'ファイルがありません' });
        const { form_id, auto_gallery } = req.body;
        if (!form_id)
            return res.status(400).json({ error: 'form_id が指定されていません' });
        const file = req.file;
        const timestamp = Date.now();
