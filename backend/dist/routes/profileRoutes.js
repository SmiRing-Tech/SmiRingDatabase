...
router.patch('/api/basic_profile_info/me', authenticate_1.authenticate, async (req, res) => {
    try {
        ...
        if (incomingMetadata && typeof incomingMetadata === 'object') {
            const { data: existingProfile, error: metadataFetchError } = await supabase_1.supabase
                .from('basic_profile_info')
                .select('metadata')
                .eq('id', req.user.id)
                .maybeSingle();
...
router.get('/api/profile/ryugakusai2026/me', authenticate_1.authenticate, async (req, res) => {
    try {
        const { data: basicInfo, error: basicError } = await supabase_1.supabase
            .from('basic_profile_info')
            .select('*')
            .eq('id', req.user.id)
            .single();
        if (basicError)
            throw basicError;
        const { data: currentStudyInfo, error: currentStudyError } = await supabase_1.supabase
            .from('current_study_abroad_profiles')
            .select('*')
            .eq('user_id', req.user.id)
            .maybeSingle();
        if (currentStudyError)
            throw currentStudyError;
        const profile = {
            ...basicInfo,
            ...(currentStudyInfo || {}),
            id: basicInfo.id // 確実にユーザーIDを保つ
        };
        const avatarUrl = await (0, r2_1.resolveAvatarUrl)(profile.avatar_id);
...
router.get('/api/profile/ryugakusai2026/:id', authenticate_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const { data: basicInfo, error: basicError } = await supabase_1.supabase
            .from('basic_profile_info')
...
router.patch('/api/profile/ryugakusai2026/me', authenticate_1.authenticate, async (req, res) => {
    try {
        const { _ai_metadata, ...updates } = req.body;
        ...
        // 最新の結合情報を取得して返す
        const { data: basicInfo, error: fetchBasicError } = await supabase_1.supabase
            .from('basic_profile_info')
            .select('*')
            .eq('id', req.user.id)
            .single();
        if (fetchBasicError || !basicInfo)
            throw fetchBasicError || new Error('Profile not found');
        const { data: currentStudyInfo, error: fetchCurrentStudyError } = await supabase_1.supabase
            .from('current_study_abroad_profiles')
            .select('*')
            .eq('user_id', req.user.id)
            .maybeSingle();
        if (fetchCurrentStudyError)
            throw fetchCurrentStudyError;
        const profile = {
            ...basicInfo,
            ...(currentStudyInfo || {}),
            id: basicInfo.id
